...
class Decoder(nn.Module):
    def __init__(self, in_size, predict_change=False):
        ...
        self.res2 = nn.Sequential(
            ...
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(64),
            # Should not squash before SEAttention as there is one already in that block
            # nn.Sigmoid(),
            SEAttention(64),
            nn.ReLU()
        )
...
if __name__ == "__main__":
    ...
    itrs_per_epoch = dataset.total_frames // batch_size
    ...
    for _epoch_itr in range(epochs):
        for _itr in range(itrs_per_epoch):
            ...
            global_step = _epoch_itr * itrs_per_epoch + _itr
            run.log({
                "loss": float(print_loss),
                "running_loss": float(running_loss),
                "running_loss_lg": float(running_loss_lg),
