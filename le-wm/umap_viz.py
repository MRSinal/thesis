"""Per-layer UMAP visualization of cached LeJEPA features (Cholec80).
...
Run with:
    python umap_viz.py --config-name cholec80 \\
        feature_cache=./cache/cholec80_features.pt
"""
...
from dataset_cholec80 import PHASE_TO_IDX
...
def run(cfg):
    ...
    feats = cache["feats"]                      # (N, L, D)
    labels = cache["labels"].numpy()
    n_total, num_layers, _ = feats.shape

    rng = np.random.default_rng(cfg.seed)
    idx = subsample(n_total, cfg.umap.max_points, rng)
    feats = feats[idx].numpy()
    labels = labels[idx]
    print(f"Subsampled to {len(idx)} / {n_total} frames; layers={num_layers}")
    ...
    for layer in range(num_layers):
        ...
        emb = reducer.fit_transform(feats[:, layer])
        out_path = figures_dir / f"umap_layer{layer:02d}.png"
        plot_layer(
            emb, labels,
            num_classes=cfg.dataset.num_classes,
            out_path=out_path,
            title=f"LeJEPA layer {layer}",
        )
